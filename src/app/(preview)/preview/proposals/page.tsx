...
import { Copy, Download, Edit2, Eye, FileText, Plus, Send, Trash2, X } from "lucide-react";
import { loadFromStorage, saveToStorage, generateId } from "@/lib/local-storage";

type ProposalSection = { id: string; title: string; content: string };
...
const SEED_PROPOSALS: Proposal[] = [
  { id: "p1", title: "Propuesta Web + SEO — TechCorp", client: "TechCorp Solutions", status: "sent", total: "$12,500 USD", createdAt: "2026-07-15", templateId: "t1", sections: [
    { id: "s1", title: "Resumen Ejecutivo", content: "Estimado Carlos Ruiz,\n\nGracias por la oportunidad de presentar nuestra propuesta. A continuación detallamos nuestra solución para TechCorp Solutions." },
    { id: "s2", title: "Alcance", content: "• Rediseño completo del sitio web\n• SEO técnico + estrategia de contenido\n• Landing pages para campañas\n• Google Ads (setup + 3 meses)" },
    { id: "s3", title: "Inversión", content: "Setup: $8,500 USD\nMantenimiento mensual: $1,500 USD\nTotal primer año: $12,500 USD" },
  ]},
  { id: "p2", title: "Consultoría Digital — MediaGroup", client: "MediaGroup Digital", status: "draft", total: "$5,000 USD", createdAt: "2026-07-17", sections: [
    { id: "s4", title: "Introducción", content: "Propuesta de consultoría para optimizar la estrategia digital de MediaGroup." },
    { id: "s5", title: "Plan", content: "4 semanas de análisis + implementación." },
    { id: "s6", title: "Inversión", content: "$5,000 USD - pago en 2 partes" },
  ]},
];
...
export default function ProposalsPage() {
  ...
  function createFromTemplate(template: Template) {
    const p: Proposal = {
      ...
      sections: template.sections.map((s) => ({ id: generateId(), title: s.title, content: s.content })),
    };
    save([p, ...proposals]);
    setEditing(p);
  ...
  function duplicateProposal(p: Proposal) {
    const dup: Proposal = { ...p, id: generateId(), title: `${p.title} (copia)`, status: "draft", createdAt: new Date().toISOString().split("T")[0]!, sections: p.sections.map((s) => ({ ...s, id: generateId() })) };
    save([dup, ...proposals]);
  }

  function addSection() {
    if (!editing) return;
    updateProposal({ ...editing, sections: [...editing.sections, { id: generateId(), title: "Nueva sección", content: "" }] });
  }
  ...
  function exportPDF(proposal: Proposal) {
    const content = `
      ...
      ${proposal.sections.map((s) => `<h2>${s.title}</h2><p>${s.content}</p>`).join("")}
